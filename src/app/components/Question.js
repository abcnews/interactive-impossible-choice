...
const Question = (state, send) => {
  ...
  return yo`<div class="Question${state.isMock ? " Question--mock" : ""}">
      ...
      state.publicGuesses && state.guess !== null
		...
		${
      state.guess !== null && state.response && state.response.length
        ? QuestionExpert(state)
        : null
    }
  </div>`;
};
...
const QuestionPersonalChoices = (state, send) => {
  return yo`<div class="QuestionPersonalChoices">
      ...
      return yo`<div class="QuestionPersonalChoices-choice">
        ...
        state.guess !== null ? "" : false
      } onclick=${send.event("guess", {
        question: state.id,
        guess: index,
      })}>${choice}</button>
			</div>`;
...
const QuestionPublicResult = (state) => {
  return yo`<div class="QuestionPublicResult">
		${
      state.publicGuessPercentages[state.guess]
  ...
  return yo`<div class="QuestionExpert">
      ...
      typeof state.source !== "object"
        ...
        : yo`<div class="QuestionExpert-source">Source:
			${
        state.source.url
          ? yo`<a href="${state.source.url}">${state.source.name}</a>`
          : state.source.name
      }
		</div>`
    }
...
export const MockQuestion = () => Question(MOCK_DATA, MOCK_SEND);
